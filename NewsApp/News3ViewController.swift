...
class News3ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UIWebViewDelegate, XMLParserDelegate {
    ...
    override func viewDidLoad() {
        ...
        imageView.image = UIImage(named: "3.jpg") // 背景画像の設定 【TODO: 独自の画像に変更する】
        ...
        // ニュースXMLの解析(パース処理)
        let url: String = "https://www.cnet.com/rss/all/" // 対象ニュースのRSS(URL)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // 画像の設定
        let urlStr = urlArray[indexPath.row].addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)! // URL文字列をエンコードする
        let url = URL(string: urlStr)! // URL型に変換
        cell.imageView?.sd_setImage(with: url, placeholderImage: UIImage(named: "placeholderImage.png")) // 画像URLをもとにimageViewに設定
...
//        let urlStr = linkURL?.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) // ★今回の追加
        let url: URL = URL(string: linkURL!)! // URL型へ変換　★今回の追加
        let urlRequest = NSURLRequest(url: url) // リクエスト
        webView.loadRequest(urlRequest as URLRequest) // URLの読み込み
    }
    ...
    func parser(_ parser: XMLParser, didStartElement elementName: String, namespaceURI: String?, qualifiedName qName: String?, attributes attributeDict: [String : String] = [:]) {
        ...
        if element == "item" {
            ...
            // RSSのmedia:thumbnailタグの画像リンクを配列に格納する
